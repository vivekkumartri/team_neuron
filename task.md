...
- [/] **Task 4H.3: Build workspace feature view, streamed agent activity, trait cards, and branch controls**
  ...
  - **Status:** Implemented `lib/generation-stream.ts`, `ActivityFeed`, `TraitCard`, `BranchControls`. **New this pass:** `components/features/workspace/WorkspaceView.tsx` composes all four into the actual workspace route — it replaces the old static `WorkspaceStudio` demo in `app/page.tsx` (whose three progression buttons had no `onClick` at all; confirmed fixed via `grep -rn "<button" web/app web/components | grep -v onClick`, which now returns zero genuinely-dead buttons). `WorkspaceView`'s `BranchControls.onSelect` now calls the real `POST /branches/:id/progression` (Task 4G.2's new `progression.py`), and once a `job_id` comes back, `useGenerationStream` opens the real SSE connection — the `/api/v1/generation-events/demo` stand-in is no longer used by the workspace route (the demo endpoint itself still exists in `app.py` for reference). `/workspace` is routed through the same `IdScopedView` text-input pattern as `/world`/`/endings`/`/reports`, since there's still no story/branch picker. **Known backend gaps, not frontend gaps:** still no TanStack Query wiring (plain hooks only), no focal-character selector, no read-only relationship graph component. `tests/e2e/generation.spec.ts` still targets the retired `WorkspaceStudio` demo's DOM shape and needs updating to match `WorkspaceView`'s actual markup before it would pass.
  - **Verification:** `npx tsc --noEmit -p web/tsconfig.json` passes. `pytest -q`: 129 passed, 18 skipped. `tests/e2e/generation.spec.ts` has not run (Playwright browser-install blocker) and is now also stale against the new component — needs a rewrite, not just an execution. Do not mark `[x]` until that spec is updated and passes.
