...
| --- | --- | --- |
| Web client | Next.js + React + TypeScript + Tailwind CSS | Responsive application shell, server/client rendering, streamed UI, accessible components. |
| API | Python FastAPI | REST endpoints, SSE streaming endpoints, auth boundary, validation, generation orchestration. |
| Authentication | OpenID Connect provider + secure HTTP-only session cookies | Establishes the tenant/owner identity; protects REST and SSE requests with the same authorization policy. |
| Async workers | Python worker processes + Redis-backed queue | Agent fan-out, retries, evaluator/business reports, memory compaction, exports later. |
| Database | PostgreSQL | Transactional story/canon data, branches, reports, agent-run audit trail, outbox events. |
...
| Observability | Structured logs, traces, metrics, `agent_runs` | Per-job lifecycle, latency, retry count, model/provider metadata, redacted input/output references. |
...
| Minimum seed length | 20 characters | Prevents unusably thin concept requests. |
...
1. Author starts a story and selects a seed type: custom prompt, dream fragment, partial narrative, or preset.
2. Author enters the seed, selects one or more genres, selects one tone, chooses an art-style preference (stored now for future media flows), reviews input, and submits.
3. The utility agent drafts three concepts. The client shows a generation state; success opens Concept Selection.
...
9. Author selects a suggested branch choice or enters a custom action. Submission creates a new child branch from the current chapter and starts the next chapter generation for that child branch.
...
stateDiagram-v2
  ...
  Published --> AwaitingChoice
  AwaitingChoice --> Queued: choice/custom input creates child branch
  Preparing --> Failed
  Deliberating --> Failed
  Writing --> Failed
  Evaluating --> Failed
...
- Selecting any choice creates a child branch. The parent chapter does not become invalid or overwritten.
...
**Components & interactions:** four-step wizard; auto-advance after seed type; 20–2,000 character prompt validation; multi-select genre; single-select tone; review step. Art-style preference is persisted but has no text-MVP dependency.

**States:** default; inline field validation; generating concepts (button disabled + status); resumable network error; no concepts generated yet; saved draft restoration after refresh.
...
**Components & interactions:** selectable cards, inline title/summary/core-conflict editing, initial-entity chips, regeneration confirmation if edits would be replaced. The selected concept is visually marked with text and icon.
...
**Hidden characteristic rule:** hidden characteristics are derived/stored only in agent-visible context at cast lock. They have no author-facing field. Do not show a blur, “secret exists” badge, or hint in roster, graph, reader, API payload, trace summary, or reports until a story reveal has been committed.
...
- Entity Graph: read-only pan/zoom/fit controls may be included, but no relationship editing. Selecting a node opens entity inspector. A list view provides equivalent data.
...
- Branch composer: choice is required; custom choice validates nonblank input and creates a child branch. The action label is “Create parallel branch & generate” rather than “Progress” to communicate the model.

**States:** no chapter yet/queued; loading before first stream event; streaming; partial scene; evaluation/regenerating; published; evaluator-blocked after retry exhaustion; pending reports; agent failure with Retry; disconnected/reconnecting SSE; empty graph; branch-selection disabled during active generation.
...
- Canon-event dialog supports kill, revive, move realm, and introduce character; includes rationale and branch target.
...
Content moderation and age-rating policy remain intentionally open for MVP. The design must therefore include a provider-agnostic `moderation_status`/policy-hook extension and a neutral UI state for `review required`. Do not claim enforcement behavior until product policy is defined. If a future policy blocks content, preserve drafts/audit data according to retention policy and show a clear, nonjudgmental reason with an allowed revision path.
